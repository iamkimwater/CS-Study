...
+ 우선순위 높은 순서로 데이터 출력
+ 평균적으로 <u>**힙**으로 구현한 우선순위 큐가 빠르다</u>

|  | 연산  |       결과     |
|---|:---:|:------------:|
| 정렬된 동적 배열 또는 링크드리스트 | `삽입`  |      O(n)    |
| 정렬된 동적 배열 또는 링크드리스트 |  `팝`  |      O(1)    |
| 힙 | `삽입`  |  **O(log n)** |
| 힙 |  `팝`  | **O(log n)** |
...
class PriorityQueue:
    ...
    # 2. 삽입한 데이터와 부모를 비교해서 위치를 조정
    # 3. 조건 만족할 때까지 2를 재귀적으로 반복
    def insert(self, data):
        self.heap.append(data)
        reverse_heapify(self.heap, len(self.heap) - 1)

    # 1. root 노드와 마지막 노드의 위치 체인지
    # 2. 마지막 노드 위치로 간 root 노드 데이터를 별도 변수에 저장, 노드는 힙에서 삭제
    # 3. 새로운 root 노드를 대상으로 heapify해서 망가진 힙 속성을 복원
    # 4. 2에서 별도 변수에 저장해 둔 최우선순위 데이터를 리턴
